...
public class DrawACardCommand extends Command {
	...
	public String execute() {
		...
		if(parts.length == 1){
			...
			info = "Están todas la filas completas, no puedes realizar el comando DRAW";
			...
			if(!anyRowAvailable){
				...
				if (card.getType().equals(E_Card.LAST_ROUND)){
					ServerMain.server.sendMessageToGamePlayers(currentGame, "Última ronda" ); //Informa de ultima ronda
